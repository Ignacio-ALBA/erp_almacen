...
$consultaTipos = "SELECT id_tipo_almacen, tipo_almacen FROM tipo_almacenes WHERE kid_estatus = 1";
$resultado = $conexion->prepare($consultaTipos);
$resultado->execute();
$tipos_almacen = $resultado->fetchAll(PDO::FETCH_ASSOC);

$tipos_almacen = array_map(fn($item) => [
  'valor' => $item['id_tipo_almacen'],
  'text' => $item['tipo_almacen'],
  'pordefecto' => 0
], $tipos_almacen);
...
CreateModalForm(
  ...
  [
    ...
    CreateInput(['type'=>'text','id'=>'almacen','etiqueta'=>'Almacén','required'=>'']),
    CreateSelect(['id'=>'id_tipo_almacen', 'etiqueta'=>'Tipo de Almacén', 'opciones'=>$tipos_almacen, 'required'=>true, 'visible'=>true, 'editable'=>false, 'soloCrear' => true]),
